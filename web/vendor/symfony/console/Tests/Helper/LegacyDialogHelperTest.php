...
class LegacyDialogHelperTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAskOnErrorOutput()
    {
        ...
        $this->assertEquals('not stdout', $dialog->ask($output = $this->getConsoleOutput($this->getOutputStream()), 'Location should output go?', 'stderr'));

        rewind($output->getErrorOutput()->getStream());
        $this->assertEquals('Location should output go?', stream_get_contents($output->getErrorOutput()->getStream()));
    }
